<?php 
	...
	//0 - For Removing the Temp image if it exists
	...
	{	
		...
		//Getting name, size and temp location		    
		...
		//Setting directory in which to be uploaded
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/social network project/assets/images/profile_pics';

		//Setting file name	
		$file_temp_name = $profile_id.'_original.'.md5(time()).'n'.$type; //temp file name
		$fullpath = $uploaddir."/".$file_temp_name; // temp file path
		$file_name = $profile_id.'_temp.jpeg'; //$profile_id.'_temp.'.$type;
		$fullpath_2 = $uploaddir."/".$file_name; //for final resized image

		//Moving file to correct location
		$move = move_uploaded_file($ImageTempName ,$fullpath) ; 
		chmod($fullpath, 0777);  
		//Checking for valid upload and displaying image and msg if successful
		if (!$move) 
		{ 
		...
		{ 
			$imgSrc = "assets/images/profile_pics/".$file_name; // image to display in crop area
		...
		//getting size of image uploaded
		clearstatcache();				
		...
		//specifying new size
		$main_width = 500; // setting image width
		$main_height = $original_height / ($original_width / $main_width);	// to set height in ratio									
		
		//creating new image using respective php func			
		...
		//uploading newly created version
		...
		@ unlink($fullpath); // to delete original upload													
	}//Image added	

	//3- Converting image To jpg and cropping
	if (isset($_POST['x']))
	{
		$type = $_POST['type'];
		...
		if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG')
		{
			...
			//creating cropped copy of image
			$img_r = imagecreatefromjpeg($src);
			$dst_r = imagecreatetruecolor( $targ_w, $targ_h );
			imagecopyresampled($dst_r, $img_r, 0, 0, $_POST['x'], $_POST['y'], $targ_w, $targ_h, $_POST['w'], $_POST['h']);
		
			//saving cropped version
...
<div class="main_column column">
    ...
    <?php
    	if($imgSrc)
    	{ 
    		//display cropping area if image has been uploaded 
    ?>
		    <script>
		    	$('#Overlay').show();
				$('#formExample').hide();
